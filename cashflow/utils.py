...
def process_xml_file(file):
    ...
    client_data = get_client_from_xml(xml_text)
    supplier_data = get_supplier_from_xml(xml_text)
    invoice_data = get_invoice_from_xml(xml_text)
    return (client_data, supplier_data, invoice_data)

def get_client_from_xml(xml_text):
...
def get_supplier_from_xml(xml_text):
    obj = xmltodict.parse(xml_text)
    df = pd.DataFrame(obj)
...
def get_invoice_from_xml(xml_text):
    obj = xmltodict.parse(xml_text)
    df = pd.DataFrame(obj)
